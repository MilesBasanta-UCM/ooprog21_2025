...
public class DebugSeven2
{
   public static void main(String[] args)
   {
      ...
      str = in.nextLine();
      length = str.length(); // len should be length
      for(x = 0; x < length; ++x) // change len to length
      {
         if(str.charAt(x) == ' ')
         {
      ...
      num = Integer.parseInt(partStr); // it should be partStr  
      System.out.println("                " + num);
      sum += num; // sum = num should be sum += num
      System.out.println("         -------------------" +
